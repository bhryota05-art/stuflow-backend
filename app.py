...
# Use SQLite in-memory database (RAM only)
...
# Auto-create tables and mock data on startup (MOVED HERE - after all models are defined)
with app.app_context():
    db.create_all()
    
    # Create mock users if they don't exist
    if not User.query.filter_by(email="teacher@stuflow.com").first():
    ...
    if not User.query.filter_by(email="student@stuflow.com").first():
    ...
    db.session.commit()
    print("✅ In-memory database initialized with mock users!")
...
def init_db():
    print("Database already auto-initialized on startup!")
